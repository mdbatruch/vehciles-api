...
class RecentlyBoughtList extends React.Component {
    ...
    render () {
        return (
            <div>
                <h1>Recently bought Vehicles for: {this.props.name.split('_').join(' ')}</h1>
                <div className="container">
                    <div className="row">
                {this.props.recentlyBoughtList.map((val, index)=> {
                    ...
                    <div className="col-md-6 col-lg-4 col-xl-3" key={index}>
                        ...
                        <div className="card-body">
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <small className="ask-price d-block">Ask Price: ${val.askPrice}</small>
                                    {(val.status == 'sold' ? 
                                        (<small className="sold-price d-block text-left text-success">Sold for: ${val.soldPrice}</small>) 
                                        : ('') 
                                    )}
                                    </div>
                                <small className="status">{val.status}</small>
                            </div>
                            <div className="d-block justify-content-between align-items-center seller">
                                {(val.status == 'sold' ? 
                                    (<small className=""><span>Sold Date:</span> { (new Date(val.soldDate)).toLocaleDateString() }</small>) 
                                    : ('') 
                                )}
...
export default RecentlyBoughtList;
